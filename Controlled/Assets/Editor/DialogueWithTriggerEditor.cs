...
public class DialogueWithTriggerEditor : Editor {
	...
	public override void OnInspectorGUI(){
		...
		for(int i = 0; i < ThisList.arraySize; i++){
			...
			SerializedProperty choicesString = MyListRef.FindPropertyRelative("dialogueChoice");
			SerializedProperty choicesInt = MyListRef.FindPropertyRelative("choicePath");
		...
		if (GUILayout.Button("Update Database")){
			GameObject.FindWithTag("DialogueDatabase").GetComponent<DialogueDB>().LoadDB(); //For making sure the database is updated (load database before updating)
